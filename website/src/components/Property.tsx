...
type TypeOrValue =
  ...
  | {
      type: 'union';
      options: [TypeOrValue, TypeOrValue, ...TypeOrValue[]];
    }
  | {
      type: 'intersect';
      options: [TypeOrValue, TypeOrValue, ...TypeOrValue[]];
...
export const Property = component$<PropertyProps>(
  ({ type, padding, ...props }: PropertyProps) => {
    return (
      <code
        class={clsx(
      ...
      >
        {typeof type === 'string' ? (
          ...
          <span class="text-purple-600 dark:text-purple-400">{type.value}</span>
        ) : type.type === 'boolean' ? (
          ...
          type.options.map((option, index) => (
            <>
              ...
              <Property type={option} padding="none" />
            </>
          ))
        ) : type.type === 'intersect' ? (
          type.options.map((option, index) => (

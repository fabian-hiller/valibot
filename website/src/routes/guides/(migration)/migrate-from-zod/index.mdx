...
> Zod recommends the use of it's `z` object export, which contains every schema class. Modern bundlers can remove ([treeshake](https://developer.mozilla.org/en-US/docs/Glossary/Tree_shaking)) unused functions and objects, but this does not work with JavaScript classes. Zod Schemas and validation chains are build using class methods,  this is one of it's disadvantages compared to me. Learn more <Link href="/guides/comparison/">here</Link>.

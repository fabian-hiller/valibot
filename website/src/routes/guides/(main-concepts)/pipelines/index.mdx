...
title: Pipelines
description: >-
  For detailed validations and transformations, a schema can be wrapped in a pipeline. This is useful to validate further details apart from the raw data type.
contributors:
  - fabian-hiller
  - ariskemper
---
...
In simple words, a pipeline is a list of schemas and actions that synchronously passes through the input data. It must always start with a schema, followed by up to 9 schemas or actions. Each schema and action can examine and modify the input. The pipeline is therefore perfect for detailed validations and transformations.
...
const NumberSchema = v.pip(v.number(), v.minValue(1234));
...
For custom validations, <Link href="/api/check/">`check`</Link> can be used. If the function passed as the first argument returns a falsy value, an issue is returned. Otherwise the input is considered valid.

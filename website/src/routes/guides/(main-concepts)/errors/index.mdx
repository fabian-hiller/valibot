...
`issues` currently only occurs when using <Link href="/api/union">`union`</Link>, <Link href="/api/unionAsync">`unionAsync`</Link>, <Link href="/api/intersection">`intersection`</Link> or <Link href="/api/intersectionAsync">`intersectionAsync`</Link> and contains all issues of the schemas of their internal types.

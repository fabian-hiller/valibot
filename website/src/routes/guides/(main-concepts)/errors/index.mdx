...
`requirement` can contain future validation information. For example, if the `minLength` validation function detects that a string is too short, `requirement` contains the minimum length that the string should have.
...
```ts
import { flatten, object, parse, string } from 'valibot'; // 1.1 kB

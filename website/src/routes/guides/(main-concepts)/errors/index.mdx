...
`issues` currently only occur when using <Link href="/api/union">`union`</Link> or <Link href="/api/unionAsync">`unionAsync`</Link> and contains all issues of the schemas of an union type.

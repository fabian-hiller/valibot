...
You can use <Link href="/api/useDefault">`useDefault`</Link> to add a default value to your schema that will be used as the input whenever `undefined` is passed.

```ts
import { parse, string, useDefault } from 'valibot'; // 0.44 kB

const StringSchema = useDefault(string(), 'hello');

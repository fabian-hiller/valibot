...
In some cases, a default value for an optional field may depend on the value of another field in the same object. This can be achieved by using <Link href="/api/pipe/">`pipe`</Link> and <Link href="/api/transform/">`transform`</Link>.
...
In this example, the `version` entry is optional. If no `version` is provided, its value is dynamically determined by the `id` entry via the `getVersionFromId` function. This allows `version` to have a default that depends on the value of `id`.

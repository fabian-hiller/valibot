...
description: >-
  There are two naming conventions that I recommend you to use when working
  with me. In this guide I will explain both of them.
contributors:
  - fabian-hiller
---
...
In the following example, a schema is created for a image object. In order to follow the naming convention, the schema and the types are exported with different names.
...
```ts
import * as v from 'valibot';
import { ImageSchema, ImageInput, ImageOutput } from './types';

---
title: pick
description: >-
  Creates an object schema that contains only the selected keys of an existing 
  schema.
source: /methods/pick/pick.ts
contributors:
  - fabian-hiller
---

import { ApiList, Property } from '~/components';
import { properties } from './properties';

# pick

Creates an object schema that contains only the selected keys of an existing schema.

```ts
// Object schema with an optional pipe
const Schema = pick<TSchema, TKeys>(schema, keys, pipe);

// Object schema with an optional message and pipe
const Schema = pick<TSchema, TKeys>(schema, keys, message, pipe);

// Object schema with rest and optional pipe
const Schema = pick<TSchema, TKeys, TRest>(schema, keys, rest, pipe);

// Object schema with rest and optional message and pipe
const Schema = pick<TSchema, TKeys, TRest>(schema, keys, rest, message, pipe);
```

## Generics

- `TSchema` <Property {...properties.TSchema} />
- `TKeys` <Property {...properties.TKeys} />
- `TRest` <Property {...properties.TRest} />

## Parameters

- `schema` <Property {...properties.schema} />
- `keys` <Property {...properties.keys} />
- `rest` <Property {...properties.rest} />
- `message` <Property {...properties.message} />
- `pipe` <Property {...properties.pipe} />

### Explanation

`pick` creates a new object schema that containing only the selected `keys` of the given object `schema`. It is similar to TypeScript's [`Pick`](https://www.typescriptlang.org/docs/handbook/utility-types.html#picktype-keys) utility type.

> Note that `rest`, `message` and `pipe` of the passed schema are completely ignored and must be added again.

## Returns

- `Schema` <Property {...properties.Schema} />

## Examples

The following examples show how `pick` can be used.

### Pick specific keys

Schema that contains only the selected keys of an existing schema.

```ts
const ObjectSchema = object({
  key1: string(),
  key2: number(),
  key3: boolean(),
});

const PickSchema = pick(ObjectSchema, ['key1', 'key2']);
```

## Related

The following APIs can be combined with `pick`.

### Schemas

<ApiList
  items={[
    'any',
    'array',
    'bigint',
    'blob',
    'boolean',
    'custom',
    'date',
    'enum_',
    'instance',
    'intersect',
    'lazy',
    'literal',
    'map',
    'nan',
    'never',
    'nonNullable',
    'nonNullish',
    'nonOptional',
    'null_',
    'nullable',
    'nullish',
    'number',
    'object',
    'optional',
    'picklist',
    'record',
    'set',
    'string',
    'symbol',
    'tuple',
    'undefined_',
    'union',
    'unknown',
    'variant',
    'void_',
  ]}
/>

### Methods

<ApiList
  items={[
    'fallback',
    'forward',
    'getDefault',
    'getDefaults',
    'getFallback',
    'getFallbacks',
    'is',
    'omit',
    'keyof',
    'parse',
    'partial',
    'required',
    'safeParse',
    'unwrap',
  ]}
/>

### Actions

<ApiList items={['check', 'brand', 'readonly', 'transform']} />

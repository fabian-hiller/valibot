...
Returns the metadata of the schema. If multiple metadata are defined, they are shallowly merged depth-first (meaning each inner pipe is deeply processed before progressing to the next pipe item). If no metadata is defined, `undefined` is returned.

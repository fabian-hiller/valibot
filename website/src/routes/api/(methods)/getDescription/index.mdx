...
Returns the description of the schema, using a reverse breadth-first search (meaning that the last description of the highest level is returned). If no description is defined, `undefined` is returned.
...
Get the description of a email schema with an overridden description.
...
const GmailSchema = v.pipe(v.string(), v.description('Gmail'), EmailSchema);

const description = v.getDescription(EmailSchema);
/*
  'Gmail'
*/
```

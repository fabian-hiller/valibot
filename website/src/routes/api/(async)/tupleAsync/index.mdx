---
title: tupleAsync
description: Creates a tuple async schema.
...
### Number and Stored email schema

Schema to validate a tuple with one number and one stored email item.

```ts
import { isEmailPresent } from '~/api';

const SimpleTupleSchema = v.tupleAsync([
  v.number(),
  v.pipeAsync(
    v.string(),
    v.email(),

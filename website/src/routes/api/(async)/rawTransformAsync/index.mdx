    ...
    async ({ dataset: { value: input }, addIssue, NEVER }) => {
      ...
      if (input.expectsFreeDelivery && total < FREE_DELIVERY_MIN_AMOUNT) {
        addIssue({
          label: 'order',
          input,
          expected: `totalAmount(cart) >= ${FREE_DELIVERY_MIN_AMOUNT}`,
          received: `totalAmount(cart) = ${total}`,
          message: `The order amount total must be at least ${FREE_DELIVERY_MIN_AMOUNT} for free delivery.`,
          path: [
            {
              type: 'object',
              input,

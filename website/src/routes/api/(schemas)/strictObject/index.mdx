...
```ts
const Schema = strictObject<TEntries, TMessage>(entries, message);
```
...
> The difference to <Link href="../object/">`object`</Link> is that this schema returns an issue for unknown entries.

...
```ts
const Schema = looseObject<TEntries, TMessage>(entries, message);
```
...
> The difference to <Link href="../object/">`object`</Link> is that this schema does include unknown entries into the output.

...
> Note: The pattern will be run using `RegExp.prototype.test()`, so you probably shouldn't use the global flag `g` on your regex pattern, as this can lead to unpredictable results. More details on [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/test#using_test_on_a_regex_with_the_global_flag)

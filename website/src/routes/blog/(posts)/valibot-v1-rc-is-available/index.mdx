...
There are many more new exciting functions like <Link href="/api/assert/">`assert`</Link>, <Link href="/api/rfcEmail/">`rfcEmail`</Link> and <Link href="/api/maxWords/">`maxWords`</Link>. Please have a look at the release on GitHub and our API reference for more details.

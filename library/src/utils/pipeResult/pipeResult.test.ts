...
describe('pipeResult', () => {
  ...
  test('should execute the pipe', () => {
    ...
    const result = pipeResult<number>(1, pipe, info, 'number');
    expect(result.output).toBe(3);
  });

  test('should return every issue', () => {
    const pipe: Pipe<number> = [minValue(5), minValue(10), minValue(15)];
    expect(pipeResult<number>(0, pipe, info, 'number').issues?.length).toBe(3);
  });
  ...
  test('should return only first issue', () => {
    ...
    expect(
      pipeResult<number>(0, pipe, infoWithAbort, 'number').issues?.length
    ).toBe(1);
  });
  ...
  test('should skip the pipeline', () => {
    ...
    expect(pipeResult<number>(0, pipe, infoWithSkip, 'number').output).toBe(0);
  });
});

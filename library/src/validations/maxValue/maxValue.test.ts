...
describe('maxValue', () => {
  test('should pass only valid values', () => {
    ...
    expect(validate1('2024').issue).toBeTruthy();
    expect(validate1('2024-01-01').issue).toBeTruthy();

    const validate2 = maxValue(3);
    expect(validate2(-3).output).toBe(-3);
    expect(validate2(0).output).toBe(0);
    expect(validate2(3).output).toBe(3);
    expect(validate2(4).issue).toBeTruthy();
    expect(validate2(456).issue).toBeTruthy();

    const validate3 = maxValue(3n);
    expect(validate3(-3n).output).toBe(-3n);
    expect(validate3(0n).output).toBe(0n);
    expect(validate3(3n).output).toBe(3n);
    expect(validate3(4n).issue).toBeTruthy();
    expect(validate3(456n).issue).toBeTruthy();
  ...
  test('should return custom error message', () => {
    ...
    expect(validate(4).issue?.message).toBe(error);
  });
});
